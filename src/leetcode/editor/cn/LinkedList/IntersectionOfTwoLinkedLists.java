...
//O(1) memory? Related Topics 哈希表 链表 双指针 👍 1772 👎 0
...
public class Solution {
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        if( lenA >= lenB ){
            return findSameNode(headA,lenA,headB,lenB);
        }else{
            return findSameNode(headB,lenB,headA,lenA);
        }
        */
        ListNode curA = headA;
        ListNode curB = headB;
        ...
        //go through own linkedlist then go through another one
        while( curA != curB){
    ...
    /*
    public ListNode findSameNode(ListNode headA, int lenA, ListNode headB, int lenB){
        ListNode curA = headA;
        ListNode curB = headB;
        int gap = lenA - lenB;
        while( gap-- > 0 ){
